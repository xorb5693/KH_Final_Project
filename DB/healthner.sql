...
ALTER TABLE PURCHASE ADD(DELIVERY_BOOL NUMBER DEFAULT 0);
ALTER TABLE PURCHASE
MODIFY DELIVERY_BOOL CONSTRAINT DELIVERY_BOOL_NOT_NULL NOT NULL;
COMMENT ON COLUMN PURCHASE.DELIVERY_BOOL IS '배송여부';

ALTER TABLE PURCHASE ADD(INVOICE_NUMBER VARCHAR2(100));
COMMENT ON COLUMN PURCHASE.INVOICE_NUMBER IS '송장번호';

ALTER TABLE MEETING_SCHEDULE ADD(BMI NUMBER);
COMMENT ON COLUMN MEETING_SCHEDULE.BMI IS '비만도';