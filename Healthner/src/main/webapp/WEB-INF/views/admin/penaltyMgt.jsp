  ...
  <body>
    ...
    <!-- Page Content  -->
       ...
       <select name="memberType">
		...
		<input type="text" id="searchWord" placeholder="아이디/이름을 입력하세요.">
	...
	<input type="button" value="권한정지" class="admin-btn" onclick="givePenalty();">
      ...
      <script>
			...
			$.ajax({
						...
						success : function(data) {
							...
							for (var i = 0; i < data.listrp.length; i++) {
								...
								html += "<td>"+data.listrp[i].boardCnt+"</td>";
								html += "<td>"+data.listrp[i].commentCnt+"</td>";
								html += "<td>"+data.listrp[i].totalReportRec+"</td>";
								html += "</tr>";
							}
		...
		function deletelist(){
			...
			$("input[name='reportSelectAll']:checked").each(function() {
				penaltyNo = $(this).parent().parent().children().eq(0).children().eq(1).val();
